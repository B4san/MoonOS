              ...
              <div className="border-t border-[var(--moon-border)] max-h-[300px] overflow-y-auto">
                {filtered.map((cmd, i) => (
                  ...
                  >
                    ...
                    <span className="text-[var(--moon-text-primary)] truncate min-w-0">{cmd.label}</span>
                    <span className="ml-auto text-[11px] text-[var(--moon-text-muted)] shrink-0 pl-3">{cmd.category}</span>
                  </button>
                ))}
              </div>
            )}
